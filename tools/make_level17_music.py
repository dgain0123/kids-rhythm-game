#!/usr/bin/env python3
"""合成第17關的背景音樂 → sounds/music/level17.m4a

第17關：BPM 90(第9關九倍速)、小朋友每 0.33 秒打一下、共 33 下。

結構（總長 16.5 秒，對齊 charts/level17.json）：
    0–1s     開頭靜音緩衝(preRollSec，躲播放起頭暫態)
    1–2.33s  預備拍：英文人聲數拍 one/two/three/four(每一拍=四分音符一聲，
             三層鏈:真人錄音>edge-tts Libby>say，見 voice_count.py)
    2.33–13s 伴奏：8 個和聲段 × 1.33 秒(C G Am F | C Am F G)＋13s 終止 C 和弦
             每 0.33 秒＝一個拍點：鐘聲走 根音(亮)→三音→五音→三音 循環
             ＋節拍器：八分音符(每 0.33 秒)一聲木魚 → 脈動不間斷
    13–16.5s 終止和弦 0.35 秒直接停後全靜音

規矩：預備拍固定 4 拍英文人聲；音樂含節拍器(細分=chart 的 metronome)。

用法：python3 tools/make_level17_music.py
"""
import os
import subprocess
import sys
...
SR = 44100
HIT = 30 / 90       # 小朋友的拍點間隔 0.333 秒(= BPM 90 的半拍)
PRE = 1.0           # 開頭靜音緩衝(躲播放起頭暫態；= chart 的 preRollSec)
COUNT = 2 * HIT     # 數拍間隔=四分音符(一拍)——速度60起八分數拍太趕(2026-07-28規矩)
LEAD_IN = PRE + 4 * COUNT  # 預備拍總長 2.333 秒(= chart 的 leadInSec)
SEG = 4 * HIT       # 一個和聲段 1.33 秒(4 個拍點換一個和弦)
...
def main():
    ...
    wav_path = os.path.join(out_dir, "_level17_tmp.wav")
    m4a_path = os.path.join(out_dir, "level17.m4a")
