#!/usr/bin/env python3
"""合成第14關的背景音樂 → sounds/music/level14.m4a

第14關：BPM 60(第9關六倍速)、小朋友每 0.5 秒打一下、共 33 下。
...
結構（總長 22.5 秒，對齊 charts/level14.json）：
    ...
    3–19s    伴奏：8 個和聲段 × 2 秒(C G Am F | C Am F G)＋19s 終止 C 和弦
             ...
             ＋節拍器：八分音符(每 0.5 秒)一聲木魚 → 脈動不間斷
    19–22.5s 終止和弦 0.35 秒直接停後全靜音

規矩：預備拍固定 4 拍英文人聲；音樂含節拍器(細分=chart 的 metronome)。

用法：python3 tools/make_level14_music.py
"""
import os
import subprocess
import sys
...
def main():
    ...
    wav_path = os.path.join(out_dir, "_level14_tmp.wav")
    m4a_path = os.path.join(out_dir, "level14.m4a")
