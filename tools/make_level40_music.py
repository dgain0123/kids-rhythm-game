#!/usr/bin/env python3
"""合成第40關的背景音樂 → sounds/music/level40.m4a

第40關（第三大關卡第11關）：BPM 100、**十六分音符 64 顆 + 1 個四分音符收尾**共 65 下，
每 0.15 秒打一下（1/4 拍）。音樂＝第三大關卡的章節風格
（自製小星星 MIDI＋SoundFont，乾聲，尼龍吉他，2026-08-14 使用者試聽選定）。

★素材速度：整章統一「**一拍＝一個拍點**」——音樂速度＝關卡速度的 4 倍
  （速度10→40BPM…速度100→400BPM）。本關 BAR = 4 × HIT = 0.6 秒 = 400BPM 4/4。
  「同章節內關卡愈快音樂就要愈快」由 tests/test_music_style.py 釘住。

★預備拍間隔＝**四分音符(一拍)一聲＝0.6 秒**（速度60起的規矩；十六分音符關卡的一拍＝4 個拍點）。

用法：python3 tools/make_level40_music.py
（素材重做：python3 tools/make_twinkle_render.py --hit 0.6 --bars 23）
"""
import os
import sys
...
CHAPTER = 3
HIT = 60 / 100 / 4  # 拍點間隔 0.15 秒(= BPM 100 的 1/4 拍)
BAR = 4 * HIT       # 素材一小節 0.6 秒 = 400BPM 4/4；一下＝一拍(track_music.render 的 source_hit)
PRE = 1.0
LEAD_HITS = 4       # 預備拍 4 聲
...
def main():
    ...
    out = os.path.join(proj, "sounds", "music", "level40.m4a")
