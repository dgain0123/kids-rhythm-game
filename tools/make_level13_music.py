#!/usr/bin/env python3
"""合成第13關的背景音樂 → sounds/music/level13.m4a

第13關：BPM 50(第9關五倍速)、小朋友每 0.6 秒打一下、共 33 下。

結構（總長 26.1 秒，對齊 charts/level13.json）：
    0–1s     開頭靜音緩衝(preRollSec，躲播放起頭暫態)
    1–3.4s   預備拍：英文人聲數拍 one/two/three/four(每 0.6 秒一聲，
             三層鏈:真人錄音>edge-tts Libby>say，見 voice_count.py)
    3.4–22.6s 伴奏：8 個和聲段 × 2.4 秒(C G Am F | C Am F G)＋22.6s 終止 C 和弦
             每 0.6 秒＝一個拍點：鐘聲走 根音(亮)→三音→五音→三音 循環
             ＋節拍器：八分音符(每 0.6 秒)一聲木魚 → 脈動不間斷
    22.6–26s 終止和弦 0.35 秒直接停後全靜音

規矩：預備拍固定 4 拍英文人聲；音樂含節拍器(細分=chart 的 metronome)。

用法：python3 tools/make_level13_music.py
"""
import os
import subprocess
import sys
...
SR = 44100
HIT = 0.6           # 小朋友的拍點間隔(= BPM 50 的半拍)
PRE = 1.0           # 開頭靜音緩衝(躲播放起頭暫態；= chart 的 preRollSec)
LEAD_IN = PRE + 4 * HIT  # 預備拍總長 3.4 秒(= chart 的 leadInSec)
SEG = 4 * HIT       # 一個和聲段 2.4 秒(4 個拍點換一個和弦)
...
def main():
    ...
    wav_path = os.path.join(out_dir, "_level13_tmp.wav")
    m4a_path = os.path.join(out_dir, "level13.m4a")
