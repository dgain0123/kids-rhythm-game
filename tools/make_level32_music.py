#!/usr/bin/env python3
"""合成第32關的背景音樂 → sounds/music/level32.m4a

第32關（第三大關卡第3關）：BPM 20、**十六分音符 32 顆 + 1 個四分音符收尾**共 33 下，
每 0.75 秒打一下（1/4 拍）。音樂＝第三大關卡的章節風格
（自製小星星 MIDI＋SoundFont，乾聲，尼龍吉他，2026-08-14 使用者試聽選定）。

★素材速度：整章統一「**一拍＝一個拍點**」——音樂速度＝關卡速度的 4 倍
  （速度10→40BPM…速度100→400BPM）。本關 BAR = 4 × HIT = 3 秒 = 80BPM 4/4。
  「同章節內關卡愈快音樂就要愈快」由 tests/test_music_style.py 釘住。

★預備拍間隔＝**拍點間隔 0.75 秒**（速度50以下的規矩）。人聲字長 0.72~0.79 秒 → 間隔比字短、字會相疊（規矩照舊不改，2026-08-04 使用者裁示）。

用法：python3 tools/make_level32_music.py
...
CHAPTER = 3
HIT = 60 / 20 / 4  # 拍點間隔 0.75 秒(= BPM 20 的 1/4 拍)
BAR = 4 * HIT       # 素材一小節 3 秒 = 80BPM 4/4；一下＝一拍(track_music.render 的 source_hit)
PRE = 1.0
LEAD_HITS = 4       # 預備拍 4 聲
...
def main():
    ...
    out = os.path.join(proj, "sounds", "music", "level32.m4a")
